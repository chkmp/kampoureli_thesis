...
trial_dur = 5.0 #default is 8 but changed it
min_dist = 40
...
filename = f"{sub_num}_{ses_num}_sa-dots_training.csv"
...
# Initialize correct count for feedback
correct_count = 0 

# Begin the blocks
for block in range(num_blocks):
    random.shuffle(num_dots_options)#shuffle for every block
    # Reset correct count for each block
    correct_count = 0
    ...
    # Provide feedback after each block
    feedback = visual.TextStim(win, text=f'You got {correct_count} out of {num_trials} trials correct.\n\nPress "1" to continue.')
    feedback.draw()
    win.flip()
    event.waitKeys(keyList=['1'])
