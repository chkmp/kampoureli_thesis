...
def thought_probe(participant_info, starttime, win, csv_info, task_name):
    ...
    ratingScale = visual.RatingScale(win, low=0, high=10, markerStart=5.0,precision=10,
                leftKeys='2', rightKeys='3', acceptKeys='1', scale = None, labels = None, acceptPreText = 'Press key', tickMarks = [1,10])
    ...
    # find the row in the dataframe where the 'item' column = nf
    # assign this row to a new variable (series)
    #remove this row from 'df', ensuring that the following loop will not present it
    # add it back to df, but at the end, ordered 
    ...
    for idx in range(len(df)):    
        ...
        while ratingScale.noResponse:
            ...
            if keyState[K_2] is True:
                pos -= inc
            elif keyState[K_3] is True:
                pos += inc
            if pos > 10: 
                pos = 10
            elif pos < 1:
