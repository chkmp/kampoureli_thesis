...
filename = f"{sub_num}_{ses_num}_n-back.csv"
...
for _ in range(n_blocks_per_condition // 2):
    random.shuffle(n_conditions)
    randomized_conditions.extend(n_conditions)
...
for n in randomized_conditions:
    ...
    for block in range(2):  # 2 blocks for each condition
        history = []
